...
> **OTT / TV 壳**：可交互元素禁止裸 `<button>` / `@click`，用 `@chancestv/tv-ui` 的 `@enter`。写页面先读 skill `chances-tv-ui` 与 rule `vue-tv-ui.md`。下面模板示例仅适用于非 TV 项目。

---

## 零、基础设施统一封装与依赖复用（强制）

跨切面能力（HTTP / 本地存储 / Cookie / 日期 / 校验 / 脱敏 / 剪贴板 等）**必须收敛到统一的基础设施层**（`core/` 或独立 composables / 工具模块），由全局唯一封装对外提供；**禁止**在业务代码里散落直接调用浏览器原生 API，也**禁止**各业务各自造轮子重复实现同类能力。

### 复用优先级

1. 项目已引入的依赖
2. 社区成熟开源方案（npm 周下载量高、近期有维护）
3. 确认无合适方案再自行实现，注释说明为何不用现成方案
...
├── modules/                    # 业务模块根目录
│   ├── users/                  # 用户功能
│   │   ├── api.ts              # 该业务的 API（用 core/http.ts 客户端）
...
通用 hooks（debounce / throttle / clickOutside / mediaQuery 等）**禁止**自己造，优先用成熟库。
...
- HTTP 客户端**必须**全局唯一，实例放 `core/http.ts`，基于成熟库封装
- **禁止**业务代码 `axios.create()` / `fetch()` / `new XMLHttpRequest()`，统一走 `core/http.ts` 导出的客户端
- token 注入 / 刷新 / 响应解包 / 错误处理统一在 `core/http.ts` 的拦截器完成，**禁止**散落到业务代码
- 每个业务模块的 API 函数集中在该业务目录的 `api.ts`，按业务域隔离
- 每个 API 函数**独立导出**，**禁止** `default export`
- 请求参数 / 响应**必须**类型标注
- 响应解包契约统一约定，在拦截器内解包后业务侧直接拿数据

### 示例

```typescript
// core/http.ts —— 全局唯一 HTTP 客户端
...
// 统一拦截器：token 注入 / 刷新 / 响应解包 / 错误分支
request.interceptors.request.use(/* 注入 token */)
request.interceptors.response.use(/* 解包 data 与统一错误处理 */)
...
- 鉴权失败 / 校验失败 / 业务错误的统一处理在 `core/http.ts` 拦截器完成
- 业务侧 catch 用统一的错误类型 / 类型守卫做分支
...
- 401 / 422 / 业务错误的拦截走 `core/http.ts` 拦截器，**禁止**每个 API 都包 try-catch
...
| 检查项 | 违规模式 | 严重程度 |
|--------|---------|---------|
| 重复造轮子 | 自造已有成熟方案的通用能力，未优先复用 | 高 |
| 基础设施散落 | 跨切面能力直接调浏览器原生 API，未收敛到统一封装 | 高 |
| 组件直调 axios | 业务代码自建 HTTP 实例 / 直接 `fetch`，未走 `core/http.ts` | **致命 → STOP** |
