...
| 持久化 | 需要持久化的状态走统一存储封装，**禁止**直接 `localStorage` |
| HTTP 调用 | 走 `core/http.ts` 全局唯一客户端，**禁止**自建 axios 实例 |
...
| 直接 localStorage | store 中直接 `localStorage.setItem(...)`，未走统一存储封装 | 高 |
| 自建 HTTP 客户端 | store 中 `axios.create()` / `fetch(...)`，未走 `core/http.ts` 客户端 | 高 |
